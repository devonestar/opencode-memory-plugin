...
export function buildCuratorPrompt(snapshot: MemorySnapshot): string {
  ...
  return [
    ...
    "Every rewrite, rescope, deletion, expired-date claim, derivability claim, ephemeral-state claim, near duplicate, contradiction, conflict, and uncertainty is report-only regardless of confidence. Emit an operation for each actionable case; use findings only for non-actionable observations and do not duplicate operations there.",
    "Do not emit KEEP operations. REWRITE, DELETE, and RESCOPE are never automatically applied. MERGE is report-only unless it is a locally provable duplicate-exact survivor operation.",
    "Emit only actionable change proposals. Omit unchanged topics, and do not repeat a source topic across operations.",
